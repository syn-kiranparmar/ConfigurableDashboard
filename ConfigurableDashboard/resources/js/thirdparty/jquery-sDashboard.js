		...
		$.widget("mn.sDashboard", {
			...
			_bindEvents : function() {
				...
				this.element.on("click", ".sDashboardWidgetHeader div.sDashboard-icon.sDashboard-circle-remove-icon ", function(e) {
					var widget = $(e.currentTarget).parents("li:first");
					$("#remove-widget-confirm").html("Are you sure you want to remove the table?");

					// Define the Dialog and its properties.
					$("#remove-widget-confirm").dialog({
						resizable: false,
						modal: true,
						title: "Remove table",
						height: 200,
						...
						buttons: {
							"Yes": function () {
								$(this).dialog('close');
								var removeUrl = "dashboard/removeWidget.json?widgetId=" + widget[0].id;
								$.ajax(
										{url: removeUrl,
										 ...
										 error: function (xhr, status) {
											alert('Error while removing widget');
										 },
